...
export default function NewProjectPage() {
  ...
  const updateTechStack = (id: string, name: string) => {
  ...
  const removeTechStack = (id: string) => {
    setTechStack(prev => prev.filter(tech => tech.id !== id));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch('/api/cms/projects', {
        ...
        body: JSON.stringify({
          ...formData,
          techStack: techStack.filter(tech => tech.name.trim()),
          metrics: metrics.filter(metric => metric.label.trim() && metric.value.trim()),
              ...
              {techStack.map((tech, index) => (
                <div key={tech.id} className="flex items-center gap-4">
                  <input
                    type="text"
                    value={tech.name}
                    onChange={(e) => updateTechStack(tech.id, e.target.value)}
